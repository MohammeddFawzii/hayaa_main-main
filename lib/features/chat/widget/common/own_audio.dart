...
class _OwnAudio extends State<OwnAudio> {
  ...
  void deleteMSG() async {
    try {
      if (widget.group == true) {
        final docRef =
            _firebaseStorage.collection("MassegeGroup").doc(widget.id);
            ...
            child: Card(
              ...
              child: Row(
                ...
                children: [
                  ...
                  Text(
                      "${formatTime(_position)}/${formatTime(_duration - _position)}"),
                ],
              ),
            ),
          ),
  ...
  String formatTime(Duration position) {
    ...
    return [if (position.inHours > 0) hour, minitus, secound].join(":");
  }
}
